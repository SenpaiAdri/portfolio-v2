    ...
    >
      <div className="relative h-full w-full">
        <div className="flex flex-col md:grid h-dvh w-full md:grid-cols-[1fr_1fr_.618fr] md:grid-rows-[0.618fr_1fr_.618fr] text-gray-400">
          {/* Header */}
          <div className="md:col-span-2 md:row-start-1 md:row-end-2 md:border-b-4 md:border-b-red-500 md:border-r-4 md:border-r-gray-600 md:border-dashed flex flex-[0.2] items-center justify-center gap-4 py-6 px-4 border-b-4 border-b-red-500 border-dashed">
              ...
              {experiences.length === 0 ? (
                ...
                <div className="space-y-6">
                  {experiences.map((exp, i) => (
                    <div key={i} className="border-l-4 border-b-4 border-gray-600 border-dashed p-4" style={{ borderColor: exp.color + '80' }}>
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
                        <h3 className="text-md tracking-[0.3em] uppercase" style={{ color: exp.color }}>{exp.role}</h3>
                        <span className="text-gray-500 text-xs tracking-wider uppercase">{exp.date}</span>
                      </div>
                      <p className="text-gray-300 text-sm mb-2 uppercase">{exp.company}</p>
          ...
          {/* Bottom tagline */}
          <div className="col-span-1 md:col-start-2 md:col-end-4 row-start-3 md:row-start-3 md:row-end-4 flex items-center justify-center px-6 py-4 border-t-4 border-t-red-500 border-dashed md:border-t-0 flex-[0.2] md:flex-auto">
            <h2 className="text-xs lg:text-2xl lg:font-black tracking-[0.2em] text-gray-500 uppercase flex items-center gap-3">
              <Briefcase className="w-4 h-4 sm:w-8 sm:h-8 text-red-500" aria-hidden="true" />
              Work History
            </h2>
          </div>
        </div>
